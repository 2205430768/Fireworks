...
public class DotAnimFW extends Dot {
  ...
  String text;
  Paint paint;
  String[] textstr={"圣","诞","快","乐"};
  int[] colorshu=new int[]{Color.RED,Color.BLACK,Color.BLUE,Color.YELLOW,Color.GREEN,Color.GRAY,Color.MAGENTA};
	/**
	 * 帧动画，利用图片实现
	 * @param context
	 * @param color
	...
	public DotAnimFW(Context context, int color, int endX, int endY) {
		...
		int colorr=rand.nextInt(7);
		colorr=colorshu[colorr];
		this.text=textstr[r];
		paint=new Paint();
	...
	class loadAnim extends Thread {
		...
		public void run() {
			...
			switch (random.nextInt(5)) {//random.nextInt(5)
			case 0:
	...
	public void myPaint(Canvas canvas, Vector<Dot> lList) {
		...
		} else if (state == 3) {//画爆炸图片
			if (mFWAnim != null) {
				if (mFWAnim.ismIsend() == false) {//没有结束
